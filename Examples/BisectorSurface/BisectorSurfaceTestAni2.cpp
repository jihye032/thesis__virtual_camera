...
BisectorSurfaceTestAni2::BisectorSurfaceTestAni2()
{
	...
	mg::GL_ResourceManager::singleton()->ImportByAssimp(scene_root_.get(), "../Data/obj/box.obj", "cup");
	...
	// pointer obj -> 그림만
	scene_root_ = std::make_unique<mg::GL_SceneRoot>();
	mg::GL_ResourceManager::singleton()->ImportByAssimp(scene_root_.get(), "../Data/obj/finger.obj", "cup");
	scene_root_->BuildSinglePolygonMesh(&mesh_joint_obj_);
	mesh_joint_obj_.ScaleUniformlyVertices(30);
	mesh_joint_obj_.TranslateVertices(cml::vector3d(-10, 50, 0));
	...
	// 카메라의 위치를 알려주기 위한 원
	mesh_cnt_.CreateSphere(6);
	mesh_cnt_.TranslateVertices(cnt_pos_);
	mesh_cnt_.Triangulate();
...
void
BisectorSurfaceTestAni2::SetCurrentTime(mg::AniTime_ms t)
{
...
void
BisectorSurfaceTestAni2::Draw(mg::AnimationViewer *w)
{
	int win_w = w->w();
	int win_h = w->h();
...
void
BisectorSurfaceTestAni2::DrawInSubViewport(int vp_x, int vp_y, int vp_w, int vp_h, int border_width = 0)
{
	...
	if (num_leap_hands_ >= 1)
	{
		...
		if (0) {
			...
			mesh_cnt_.Clear();
			mesh_cnt_.CreateSphere(8);
			mesh_cnt_.TranslateVertices(cnt_pos_);
			mesh_cnt_.Triangulate();
			renderer_->SetColor(1, 0, 0);
			renderer_->DrawMesh(&mesh_cnt_);
		}
		...
		if (0) {
			...
			for (int i = 0; i < b_sites.size(); i++)
			{
				...
				renderer_->glVertex(mesh_B_.vertex(b_sites[i].first));
			}
			renderer_->glEnd();
			...
			for (int i = 0; i < b_sites.size(); i++)
			{
				renderer_->glVertex(mesh_joint_obj_.vertex(b_sites[i].second - mesh_B_.num_vertices()));
			}
			renderer_->glEnd();
		...
		if (1)
		{
			...
			renderer_->SetColor(0, 0, 0);
			renderer_->glBegin(GL_POINTS);
			for (int i = 0; i < mesh_joint_obj_.num_vertices(); i++)
			{
				renderer_->glVertex(mesh_joint_obj_.vertex(i));
			}
			renderer_->glEnd();
			...
			renderer_->SetColor(0, 0, 0);
			renderer_->glBegin(GL_POINTS);
			for (int i = 0; i < mesh_B_.num_vertices(); i++)
			{
				renderer_->glVertex(mesh_B_.vertex(i));
			}
			renderer_->glEnd();
		}

		std::cout << mesh_B_.num_vertices() + mesh_joint_obj_.num_vertices() << std::endl;
		std::cout << mesh_B_.num_vertices() << std::endl;
		std::cout << mesh_joint_obj_.num_vertices() << std::endl;
...
void
BisectorSurfaceTestAni2::DrawInSubViewport2(int vp_x, int vp_y, int vp_w, int vp_h, int border_width = 0)
{
...
void
BisectorSurfaceTestAni2::DrawBisector()
{
	const std::vector<cml::vector3d>&      b_vertices = b_surface_.voronoi_vertices();
	const std::vector< std::vector<int> >& b_faces = b_surface_.bisector_faces();
	const std::vector<cml::vector3d>&      b_normals = b_surface_.bisector_faces_normals();
	...
	if (false)
	{
		...
		for (unsigned int i = 0; i < b_sites.size() && i < b_faces.size(); i++)
		{
			...
			renderer_->glVertex(b_surface_.GetVertexOfB(b_sites[i].second - mesh_B_.num_vertices()));
		}
		renderer_->glEnd();
	}
...
cml::vector3d
BisectorSurfaceTestAni2::FindPreSubCamera()
{
	const std::vector<cml::vector3d>&      b_vertices = b_surface_.voronoi_vertices();
	const std::vector< std::vector<int> >& b_faces = b_surface_.bisector_faces();
	const std::vector<cml::vector3d>&      b_normals = b_surface_.bisector_faces_normals();
	...
	for (int i = 0; i < b_sites.size(); i++)
	{
		...
		cml::vector3d b = mesh_joint_obj_.vertex(b_sites[i].second);
...
cml::vector3d
BisectorSurfaceTestAni2::FindSubCameraPosition()
{
	const std::vector<cml::vector3d>&      b_vertices = b_surface_.voronoi_vertices();
	const std::vector< std::vector<int> >& b_faces = b_surface_.bisector_faces();
	const std::vector<cml::vector3d>&      b_normals = b_surface_.bisector_faces_normals();
	...
	for (int i = 0; i < pre_num_ - 1; i++)
	{
		...
		double dd = DotProduct(cml::normalize(cmr_eye), cml::normalize(tmp));
...
void BisectorSurfaceTestAni2::onInit(const Leap::Controller& controller) {
...
void BisectorSurfaceTestAni2::onConnect(const Leap::Controller& controller) {
std::cout << "Connected" << std::endl;
}

void BisectorSurfaceTestAni2::onDisconnect(const Leap::Controller& controller) {
// Note: not dispatched when running in a debugger.
std::cout << "Disconnected" << std::endl;
}

void BisectorSurfaceTestAni2::onExit(const Leap::Controller& controller) {
std::cout << "Exited" << std::endl;
}

void BisectorSurfaceTestAni2::onFrame(const Leap::Controller& controller) {
...
void BisectorSurfaceTestAni2::onFocusGained(const Leap::Controller& controller) {
std::cout << "Focus Gained" << std::endl;
}

void BisectorSurfaceTestAni2::onFocusLost(const Leap::Controller& controller) {
std::cout << "Focus Lost" << std::endl;
}

void BisectorSurfaceTestAni2::onDeviceChange(const Leap::Controller& controller) {
...
void BisectorSurfaceTestAni2::onServiceConnect(const Leap::Controller& controller) {
std::cout << "Service Connected" << std::endl;
}

void BisectorSurfaceTestAni2::onServiceDisconnect(const Leap::Controller& controller) {
std::cout << "Service Disconnected" << std::endl;
}

void BisectorSurfaceTestAni2::onServiceChange(const Leap::Controller& controller) {
std::cout << "Service Changed" << std::endl;
}

void BisectorSurfaceTestAni2::onDeviceFailure(const Leap::Controller& controller) {
...
void BisectorSurfaceTestAni2::onLogMessage(const Leap::Controller&, Leap::MessageSeverity s, int64_t t, const char* msg) {
switch (s) {
